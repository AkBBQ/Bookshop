...
<table class="table table-bordered table-striped table-hover">
    	...
    	<c:forEach items="${list}" var="t" varStatus = "status">
    		...
    		<td>${status.count}</td>
    			...
    			<input type="hidden" id="OrderId" value="${t.orderId}">
...
<div class="form-group" style="text-align: center">
            	<button  id="SendGoods" class="btn-info btn-lg">发货</button>        	
        	</div>
<%-- <div>
...
$('#SendGoods').click(function(){
	var id=${list.get(0).orderId};
	var status1=1;//发货将status状态设置为1
	var sendData="";

	$.ajax({
		method:'post',
		url:'orders/sendGoods',
		data:{status:status1,shipDate:sendData,orderId:id},
		success:function(result){
			alert("发货成功!");
			window.location.href="orders/showAll";
		},
		error:function(){
